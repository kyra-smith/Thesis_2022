...
load(file = paste0(outDir, "Map_data_tempvars_2016_18_thresh_10_FUTURE.rdata"))
...
nrow(temp_data[temp_data$StdAnom >12, ]) # 1505 rows
temp_data <- temp_data[temp_data$StdAnom < 12, ] # 62706 rows

cor(temp_data$avg_temp, temp_data$StdAnom) # -0.054, Future, thresh 10
...
cor(temp_data$Anom, temp_data$StdAnom) # 0.066, Future, thresh 10
...
cor_temp <- round(cor(SP_df[SP_df$Tropical == "Temperate", "Anom"], SP_df[SP_df$Tropical == "Temperate", "StdAnom"]), digits = 2) # ????, Future
